...
>
> **接手第一件事**：读 `docs/12-待办与发布清单.md` —— 当前进度快照、没做完的事、
> 哪些只有用户能在后台做（AdMob/Play），都在那份清单里。
...
## 7. 还没做的事 → 全量清单在 `docs/12-待办与发布清单.md`

换机器/换会话接手时**先读 `docs/12`**：里面有状态快照（版本、真机、关键 ID）、
发布前必做项、代码侧待办、以及"哪些只有用户能在后台做"。这里只放最紧急的几条：

1. **发布阻塞项**：真图标/自适应图标/启动图、`values-en` 英文文案、商店素材（截图/feature graphic/
   描述）、Data safety、隐私政策公开网址；AdMob 侧还需要用户补齐**付款资料**
   （不补齐应用审核会一直卡在 `Getting ready`，见 `docs/12` A1）与 **GDPR 同意消息**。
2. **release 包真实广告回归**：真实 ID 只在 release 生效，需用户跑 `scripts\build-direct-release.ps1`
   后装包验证（顺带验证 R8 没削坏 SDK）。
3. 体验：媒体库页内嵌浏览（替代对话框）、本机播放的通知栏媒体控制 + 前台服务。
4. 变现：激励视频 / 去广告内购（接 `AdPolicy` + `FeatureId.NO_ADS`）、服务器助手（收费方向）。
...
| `docs/12-待办与发布清单.md` | **接手先读**：状态快照与关键 ID、发布前必做（含只有用户能做的后台操作）、代码侧待办、换机开工步骤 |
